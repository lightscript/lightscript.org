...
-x await
  -x await-equals
    -x const, MemberExpression, now
  -x naked await, return await
  -x await array
  -x safe-await
-x property access
  -x SafeMemberExpression
    -x ?.
    -x ?[
  -x Array dot access (array.0)
  -x function property definition
-x tilde-calls
  -x basic example
  -x chaining example
-x Objects
...
-x Array Comprehensions
  ...
  -x matrix
-x for loops
  -x (intro on why arrays suck in js)
  -x for-in auto-const
  -x for-of auto-const
  -x for-from array
    -x index
...
-x switch
-x Classes
  -x support for ES7 features
  -x { or :
  -x methods
  -x bound methods
  -x constructors
    -x constructor() insertion
    -x super() insertion
      -x arguments
-x Automatic Semicolon Insertion
  -x `+` and `-`, binary and unary
  -x `/`, division and regular expression
  -x `(`, expressions and function calls
  -x `<`, less-than and JSX
  -x `[`, index-access and arrays
-x Inconsistencies, Ambiguities, and Known Bugs
-x Deviations from JavaScript
  -x `==`, `!=`
  -x `~`
  -x keywords
    -x `now`, `or`, `and`, `not`, `til`, `thru`, `from`(?)
  -x no invisible characters
  -x ASI. "broken" things
  -x blocks vs objects
